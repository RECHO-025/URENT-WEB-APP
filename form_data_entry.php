...
<div class="form-container">
  ...
  <form action="index.php?page=expense_report" method="post" name="form1" id="form1">
    ...
    <div class="form-group">
      ...
      <select id="reporttype" name="reporttype" required>
        ...
        <option value="General">Landlord report (All Sites)</option>
        <?php 
          $houses = $conn->query("SELECT * FROM houses WHERE shopid ='$landlordid' ORDER BY id ASC");
          if($houses->num_rows > 0):
            while($row = $houses->fetch_assoc()):
        ?>
          <option value="<?php echo $row['house_no'] ?>"><?php echo $row['house_no'] ?> : <?php echo $row['description'] ?></option>
        <?php 
            endwhile;
          else: 
        ?>
        ...
        <option value="">Or sort by site</option>
        <?php 

				...
				<div class="col-md-12">
					<form id="filter-report">
						<div class="row form-group">
							...
							<label class="control-label col-md-2 text-right">Filter Status: </label>
							<select name="status_filter" class="form-control col-md-2">
								<option value="">All Status</option>
								<option value="1" <?php echo $status_filter == '1' ? 'selected' : '' ?>>Paid</option>
								<option value="0" <?php echo $status_filter == '0' ? 'selected' : '' ?>>Not Paid</option>
							</select>
					...
					<div id="report">
						<div class="on-print">
							 ...
							 <?php if(!empty($status_filter)): ?>
							     <p><center>Filtered by Status: <b><?php echo $status_filter == '1' ? 'Paid' : 'Not Paid' ?></b></center></p>
							 <?php endif; ?>
						</div>
						<div class="row">
							<table class="table table-bordered">
								...
								<tbody>
									...
									$i = 1;
									$tamount = 0;
									$payment_query = "SELECT p.id AS payment_id, 
       p.tenant_id, 
       p.amount, 
       p.InvoiceType, 
       p.Year, 
...
FROM payments p WHERE date_format(p.date_created, '%Y-%m') = '$month_of' 
  AND p.shopid = '$landlordid'";
									...
									if(!empty($status_filter)) {
									    $payment_query .= " AND p.pay_status = '$status_filter'";
									}
								...
								<?php else: ?>
									<tr>
										<th colspan="10"><center>No Data.</center></th>
									</tr>
								<?php endif; ?>
								</tbody>
								<tfoot>
